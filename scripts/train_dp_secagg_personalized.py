"""E6's "Ours+DP+SecAgg" comparator -- client-level DP with a FIXED
public clip bound (not Phase 8's per-round adaptive median, which needs
server-side visibility into individual client update norms that SecAgg+
specifically hides) combined with real SecAgg+ secure summation. See
DPSecAggPersonalizedFedAvg's and run_dp_secagg_personalized_simulation's
docstrings in simulation.py for the full mechanism, and client.py's
dp_clip_norm docstring for the client-side half.

--target-epsilon=inf is still meaningful here (unlike the plain DP
script) -- clients keep clipping to --clip-bound regardless of noise, so
eps=inf is a real, distinct point (SecAgg+ + fixed clipping, no noise),
not identical to plain SecAgg+ (scripts/train_secagg_personalized.py).

Usage:
    # smoke test: few rounds, before committing to a full 100-round run
    python scripts/train_dp_secagg_personalized.py --dataset cicids2017 --alpha 5 \
        --target-epsilon 3 --rounds 3 --run-tag smoketest

    # full run (one point of E6's Table T6)
    python scripts/train_dp_secagg_personalized.py --dataset cicids2017 --alpha 0.5 --target-epsilon 3
"""
...
from fedpda_ids.federated.simulation import run_dp_secagg_personalized_simulation  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="E6: DP + SecAgg+ combined personalized FL")
    ...
    parser.add_argument("--clip-bound", type=float, default=5.0,
                         help="fixed public per-client clip bound (default: median adaptive clip norm "
                              "observed in Phase 8's real completed CICIDS2017 alpha=5/0.5 sweeps)")
    ...
    logger = setup_logging(
        ...
        log_to_file=config["logging"]["log_to_file"], run_name="train_dp_secagg_personalized",
    )
    ...
    eps_tag = "inf" if args.target_epsilon == float("inf") else str(args.target_epsilon)
    run_name = f"{args.dataset}_{scope_desc}_dp_secagg_personalized_eps{eps_tag}"
    if args.run_tag:
    ...
    logger.info(
        "Starting DP+SecAgg+ personalized FL: run=%s clients=%d/round clients_configured=%d rounds=%d "
        "target_epsilon=%s clip_bound=%.4f",
        run_name, clients_per_round, num_clients_configured, num_rounds, args.target_epsilon, args.clip_bound,
    )
    ...
    result = run_dp_secagg_personalized_simulation(
        ...
        target_epsilon=args.target_epsilon, target_delta=target_delta, clip_bound=args.clip_bound,
    ...
    print(f"  target_epsilon={args.target_epsilon} achieved_epsilon={result['achieved_epsilon']:.4f} clip_bound={args.clip_bound}")
