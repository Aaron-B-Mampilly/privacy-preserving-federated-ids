"""Phase 9: SecAgg+ on top of Phase 6's plain (non-DP) personalized FL.

SecAgg+ cryptographically sums client updates so the server never sees
any individual client's shared-encoder/decoder update -- only the
final (correctly-averaged) result. Evaluated independently from Phase
8's DP mechanism (adaptive clipping needs to SEE individual updates,
which SecAgg+ specifically prevents -- see simulation.py's module
docstring for the full reasoning and the user's decision to keep these
two mechanisms separate rather than combine them).

Usage:
    # smoke test: few rounds, before committing to a full 100-round run
    python scripts/train_secagg_personalized.py --dataset cicids2017 --alpha 5 --rounds 3 --run-tag smoketest

    # full run
    python scripts/train_secagg_personalized.py --dataset cicids2017 --alpha 5
    python scripts/train_secagg_personalized.py --dataset nbaiot --scheme main_45
"""
...
from fedpda_ids.federated.simulation import run_secagg_personalized_simulation  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 9: SecAgg+ personalized FL")
    ...
    logger = setup_logging(
        ...
        log_to_file=config["logging"]["log_to_file"], run_name="train_secagg_personalized",
    )
    ...
    run_name = f"{args.dataset}_{scope_desc}_secagg_personalized"
    if args.run_tag:
    ...
    logger.info(
        "Starting SecAgg+ personalized FL: run=%s clients=%d/round clients_configured=%d rounds=%d",
