...
from fedpda_ids.federated.simulation import run_fedavg_simulation  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Phase 5: Flower FedAvg baseline")
    ...
    logger = setup_logging(
        ...
        log_to_file=config["logging"]["log_to_file"], run_name="train_federated",
    )
    ...
    logger.info(
        "Starting FedAvg: run=%s clients=%d/round clients_configured=%d rounds=%d",
    ...
    summary = {
        ...
        "best_round": result["best_round"], "best_val_loss": result["best_val_loss"],
    ...
    print(f"  test accuracy={result['test_metrics']['accuracy']:.4f} macro_f1={result['test_metrics']['macro_f1']:.4f}")
    print(f"  best round={result['best_round']} best_val_loss={result['best_val_loss']:.4f}")
